...
export const Player: React.FC<PlayerProps> = ({ channel, allChannels, onSelectChannel, onFullscreenChange }) => {
  ...
  const selectedChannelRef = useRef<HTMLButtonElement>(null);
  ...
  const nowPlaying = channel.epg[0];
  const nextUp = channel.epg[1];
           ...
           {nowPlaying && (
             <div className="mt-2 md:mt-4 max-w-2xl">
                <div className="flex items-center space-x-3">
                    <span className="text-xs font-bold text-red-400 bg-red-900/50 px-2 py-1 rounded-full uppercase">LIVE</span>
                    <span className="font-semibold text-sm md:text-base">{nowPlaying.startTime} - {nowPlaying.endTime}</span>
                </div>
                <h3 className="text-lg md:text-2xl lg:text-3xl mt-1 font-semibold drop-shadow-md">{nowPlaying.title}</h3>
                {nextUp && (
                    <p className="text-gray-300 mt-2 text-sm md:text-base lg:text-lg">
                        <span className="font-semibold text-gray-200">Next:</span> {nextUp.startTime} - {nextUp.title}
                    </p>
                )}
             </div>
           )}
